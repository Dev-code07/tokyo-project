...
export default function Header() {
  ...
  return (
    <motion.header
      ...
      className={`sticky top-0 left-0 w-full z-40 transition-all duration-500 sm:my-5 ${scrolled ? "bg-[#111111]/95 backdrop-blur-md" : "bg-transparent"
        }`}
    >
      <div className="max-w-[1300px] mx-auto flex justify-between items-center sm:px-6 px-4 h-16">
        ...
        >
          ...
          >
            <Image
              src="/assets/gifs/logo.svg" 
              alt="ION Logo"
              width={230} 
              height={71}
              className="sm:w-full w-[150px] h-auto" 
              priority
            />
          </Link>
        </motion.div>
        ...
        <nav className="hidden xl:flex space-x-8 text-[18px] tracking-tighter items-center">
          {navLinks.map((link, i) => (
            ...
            >
              <Link
                href={link.href}
                className={`transition-colors uppercase relative ${pathname === link.href
                    ? "text-blue-500"
            ...
            <nav className="flex flex-col justify-center space-y-6 p-6 min-h-[100vh] bg-[#111111]">
              {navLinks.map((link, i) => (
                ...
                >
                  <Link
                    href={link.href}
                    className={`transition-colors text-[24px] ml-10 font-bold ${pathname === link.href
                        ? "text-blue-500"
